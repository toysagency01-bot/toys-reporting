...
function buildChannelTabs(sheetTitles, label){
  const spacedSuffix = ' (' + label + ')';
  const compactSuffix = WEEKLY_PROJECT_KEY === 'housevip-cxp7' ? '(' + label + ')' : '';
  return sheetTitles
    .filter(t => t.endsWith(spacedSuffix) || (compactSuffix && t.endsWith(compactSuffix)))
    .map(t => {
      const suffix = t.endsWith(spacedSuffix) ? spacedSuffix : compactSuffix;
      const base = t.slice(0, -suffix.length);
      const mode = WEEKLY_PROJECT_KEY === 'housevip-cxp7' && (base === 'ЛИДЫ' || base === 'Лиды')
        ? { mode: 'lead-feedback' }
        : (CHANNEL_TAB_MODES[base] || {});
      return Object.assign({ tab: t, label: base }, mode);
    })
